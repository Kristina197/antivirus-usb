...
VirusScanner::VirusScanner(std::shared_ptr<ISignatureRepository> sigRepo,
                           std::shared_ptr<QuarantineManager> quarantineManager)
    : sigRepo_(sigRepo), quarantineManager_(quarantineManager) {}
    ...
    try {
        for (const auto& entry : fs::recursive_directory_iterator(path)) {
            if (entry.is_regular_file()) {
                scanFile(entry.path().string(), results);
            }
        }
...
bool VirusScanner::scanFile(const std::string& filePath, std::vector<ScanResult>& results) {
    ...
    for (const auto& sig : signatures_) {
        ...
        if (hash == sig.signatureHash) {
            ...
            std::cout << "⚠ THREAT DETECTED: " << filePath << " -> " << sig.virusName << "\n";
            
            // УБРАЛИ автоматический карантин - теперь будет через GUI
